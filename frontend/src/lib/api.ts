...
api.interceptors.response.use(r=>r,async e=>{const o=e.config;if(e.response?.status===401&&!o._retry){o._retry=true;try{await axios.post(`${apiBase}/auth/refresh`,{},{withCredentials:true});return api(o)}catch(x){if(typeof window!=='undefined'&&!window.location.pathname.startsWith('/admin'))window.location.href='/admin';return Promise.reject(x)}}return Promise.reject(e)});
