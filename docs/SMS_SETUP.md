...
این راهنما نحوه پیکربندی ارسال پیامک تأیید (OTP) در محیط تولید (Production) را برای پلتفرم **آین تراز** شرح می‌دهد.
...
سرویس SMS آین تراز (`backend/src/modules/auth/sms.service.ts`) یک درخواست POST به `SMS_API_URL` ارسال می‌کند با ساختار زیر:
...

{
  ...
  "mobile": "09123456789",
  "template": 1,
  "message": "کد ورود به آین تراز: 123456\n\nآین تراز\nمشاوره مالیاتی و حسابداری"
}
```
...
|-----------|-------------|--------|
| 0 | `کد: ` | عمومی |
| 1 | `کد ورود به آین تراز: ` | ورود به سیستم (پیش‌فرض) |
| 2 | `کد تأیید آین تراز: ` | تأیید عملیات |
| 3 | `رمز عبور موقت آین تراز: ` | بازنشانی رمز |
| 4 | `رمز ورود آین تراز: ` | رمز ورود دومرحله‌ای |
...
کاوه‌نگار از قالب‌های از پیش تعریف شده (Pattern) استفاده می‌کند. نیاز به یک لایه میانی (Middleware) دارید که درخواست آین تراز را به فرمت کاوه‌نگار تبدیل کند:
...
```javascript
// middleware.js — بین آین تراز و کاوه‌نگار
...
> **توجه:** اکثر سرویس‌های پیامک ایرانی از قالب‌های OTP با متغیر (Pattern-based) استفاده می‌کنند که فرمت متفاوتی از درخواست مستقیم آین تراز دارند. استفاده از لایه میانی توصیه می‌شود.
